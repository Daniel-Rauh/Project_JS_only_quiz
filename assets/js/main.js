...
data.forEach((elt) => {
    answerArr.push(elt.answer.toString())
    content.innerHTML += `<img src=${elt.url} alt="">
    <h3>${elt.question}</h3>
    <div id ="choices${data.indexOf(elt)}"></div>`
    for (let i = 0; i < elt.choice.length; i++) {
        let choices = document.getElementById(`choices${data.indexOf(elt)}`)
        choices.innerHTML += `<button onclick="answer('${elt.choice[i]}')" id = "${elt.choice[i]}">${elt.choice[i]}</button>`
    }
})
function answer(guess) {
    count++
    counter.innerHTML = `Tries: ${count}`
    let parent = document.getElementById(guess).parentElement
    if (answerArr.includes(guess) == true) {
